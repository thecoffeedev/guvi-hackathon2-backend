...
app.post('/add-to-cart', tokenValidation, async (req, res) => {
    try {
        let client = await mongoClient.connect(dbUrl);
        let db = client.db('equirent')
        let item = [
        ...
        let result = await db.collection('users').findOneAndUpdate({_id: objectId(req.body.id)}, {$push: {cart: item}})
...
app.post('/remove-from-cart', tokenValidation, async (req, res) => {
    try {
        let client = await mongoClient.connect(dbUrl);
        let db = client.db('equirent')
        // let item = [
        ...
        let result = await db.collection('users').findOneAndUpdate({_id: objectId(req.body.id)}, {$pull: {cart: { $elemMatch: {item_id: req.body.item_id}}}})
...
app.post('/add-purchase', tokenValidation, async (req, res) => {
    try {
        let client = await mongoClient.connect(dbUrl)
        let db = client.db('equirent');
        // let purchase = [
        ...
        //     }
        // ]
        let result = await db.collection('users').findOneAndUpdate({_id: objectId(req.body.id)}, {$push: {purchases: req.body.purchase}})
        await db.collection('purchases').insertOne(req.body.purchase)
...
app.post('/add-product', [tokenValidation, checkRole('ADMIN')], async (req, res) => {
    try {
        let client = await mongoClient.connect(dbUrl)
        let db = client.db('equirent')
